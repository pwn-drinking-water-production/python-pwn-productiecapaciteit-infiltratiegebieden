...
class strangWeerstand(object):
    ...
    def __repr__(self, width=100):
        ...
        object_methods = [
            method_name for method_name in dir(self) if callable(getattr(self, method_name)) and method_name[0] != "_"
        ]
        si = "METHODS:\n - " + "\n - ".join(object_methods)
        s.append(si)
    ...
    def capaciteit_effect_schoonmaak(self, date_clean, date_test, leiding=True, wel=True):
        cap_na_schoonmaak = self.capaciteit_schoonmaak(date_clean, date_test, leiding=leiding, wel=wel)

        return cap_na_schoonmaak - self.capaciteit(date_test)

    def report_capaciteit_effect_schoonmaak(self, date_clean, date_goal):
        """
        Investigate the effect a schoonmaak on date `date_clean` on the capacity on date `date_goal`.
        """
        effect_leiding = self.capaciteit_effect_schoonmaak(date_clean, date_goal, leiding=True, wel=False)
        effect_wel = self.capaciteit_effect_schoonmaak(date_clean, date_goal, leiding=False, wel=True)
        effect_som = self.capaciteit_effect_schoonmaak(date_clean, date_goal, leiding=True, wel=True)
    ...
    def capaciteit_effect_schoonmaak_cat(self, date_clean, date_test, leiding=True, wel=True):
        strang2 = self.get_schoonmaakscenario(date_clean, leiding=leiding, wel=wel)
    ...
    def plot_lims(self, index, date_clean, ax=None, deltah_veilig=1.5):
        ...
        ax.plot(index_voor, flow_min_voor, label="Min. inzet voor schoonmaak", lw=1, c=f"C{nlims + 2}", ls="--")
        ax.plot(index_na, flow_min_na, lw=2, c=f"C{nlims + 2}", ls="--", alpha=0.3)

        # After cleaning
        flow_min_na = self.lim_flow_min_schoonmaak(index_na, date_clean, leiding=True, wel=True)
        ax.plot(index_na, flow_min_na, label="Min. inzet na schoonmaak", lw=1, c=f"C{nlims + 3}", ls="--")
        ...
        self.plot_lim_cat(
            date_clean, index, ax=ax, y1=0.05, y2=0.095, leiding=False, wel=False, info="Voor schoonmaak", legend=True
        )
        self.plot_lim_cat(
            date_clean,
            index_na,
    ...
    def plot_lim_cat(
        ...
        lim_cat = self.capaciteit_effect_schoonmaak_cat(date_clean, date_test, leiding=leiding, wel=wel)
